...
<div id="kanban-board-container" class="flex-1 overflow-x-auto flex gap-3.5 items-stretch pb-1 w-full min-h-0">
    @foreach($kanbanColumns as $colId => $col)
        <div class="kanban-column-wrapper flex flex-col bg-base-200/50 rounded-xl border border-base-300 shadow-xs transition-all duration-200 shrink-0 w-80 h-full max-h-full overflow-hidden" 
            ...
            <div class="kanban-column-header bg-base-200/90 p-2.5 flex flex-col gap-1.5 border-b border-base-300 rounded-t-xl shrink-0 {{ $col['border_color'] }}">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-0.5">
                        ...
                        <!-- Delete Column Button (Only visible if column is empty) -->
                        <button type="button" class="btn btn-ghost btn-xs text-error/70 hover:text-error col-delete-btn ml-0.5 {{ $col['count'] > 0 ? 'hidden' : '' }}" data-slug="{{ $colId }}" title="Excluir Coluna (Sem cards)">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="kanban-column-body flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2.5 p-2.5 min-h-0" 
                data-stage="{{ $colId }}"
                data-page="1"
                data-has-more="{{ count($col['items']) < $col['count'] ? 'true' : 'false' }}">
                ...
                <div class="cards-list-wrapper flex flex-col gap-2.5">
                    @foreach($col['items'] as $cliente)
                        @include('pages.clientes.partials.kanban-card', ['cliente' => $cliente, 'colId' => $colId, 'kanbanColumns' => $kanbanColumns, 'tab' => $tab])
                    @endforeach
                </div>

                <!-- Column Lazy Loading Spinner -->
                <div class="column-loading-spinner hidden py-3 flex justify-center items-center">
                    <span class="loading loading-spinner loading-xs text-primary"></span>
                    <span class="text-[11px] text-base-content/50 ml-1.5 font-medium">Carregando mais...</span>
                </div>
            </div>
        </div>
    @endforeach
